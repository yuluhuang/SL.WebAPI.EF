    ...
    */
    public class LoginController : ODataController
    {
        ...
        public IHttpActionResult GetLogin(User user)
        {
            ...
            {
                string usernamehash = GetMd5Hash(md5Hash, user.userId);//将用户编号hash
              User  dbuser=  db.userTable.First(x => x.userId == user.userId);//获取数据库内用户对象
              string userpassword = passwordToMd5(dbuser.password, dbuser.salt);//将用户输入密码加盐hash[与数据库中获取的密码比较]
              if (userpassword.Equals(dbuser.password))
              {
                  return null;// usernamehash;//+ pands[2];
              }
              else
              {
                  return null;
