       ...
       */
        ...
        public IHttpActionResult Post(User user)
        {
            ...
            using (MD5 md5Hash = MD5.Create())
            {
                ...
                s = db.userTable.First(x => x.userId == s.userId);

                user.indentity = s.indentity;
                return Json(user);
            }
        ...
        private bool UserExists(string key)
        {
            using (MD5 md5Hash = MD5.Create())
            {
                string hash = encodeAndDecode.GetMd5Hash(md5Hash, key);
                return db.userTable.Count(e => e.userId == hash) > 0;
            }
        }
